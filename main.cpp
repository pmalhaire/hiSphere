...
// Whether we should quit. It is most convenient for this to be filescope right
// now.
...
void log_gl_value(const std::string &name, SDL_GLattr sdl_enum) {
  int val;
  if (SDL_GL_GetAttribute(sdl_enum, &val)) {
    std::cerr << "(error?) ";
  }
...
void output_gl_diagnostics() {
  ...
  if (!(gl_version && gl_renderer && gl_vendor && glsl_version)) {
    std::cerr << "Could not obtain complete GL version info... GL context "
                 "might not exist?!?\n";
  }

  std::cout << align("GL version:", 3) << (gl_version ? gl_version : "(null)")
            << std::endl;
  std::cout << align("GL renderer:", 3)
            << (gl_renderer ? gl_renderer : "(null)") << std::endl;
  std::cout << align("GL vendor:", 3) << (gl_vendor ? gl_vendor : "(null)")
            << std::endl;
  std::cout << align("GLSL version:", 3)
            << (glsl_version ? glsl_version : "(null)") << std::endl;
  std::cout << align("GL extensions:", 3)
            << (gl_extensions ? gl_extensions : "(null)") << std::endl;
...
struct SDL_graphics {
  ...
  SDL_graphics(int width, int height)
      : width_(width), height_(height), window_(nullptr), context_(nullptr)
  //		, renderer_(nullptr)
  {
    ...
    window_ = SDL_CreateWindow(
        "DEMO", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height,
        SDL_WINDOW_OPENGL |
            SDL_WINDOW_RESIZABLE /*| SDL_WINDOW_ALLOW_HIGHDPI*/);
    if (window_ == nullptr) {
      std::cerr << "Error when creating SDL GL Window: '" << SDL_GetError()
                << "'\n";
    }

    context_ = SDL_GL_CreateContext(window_);
    if (!context_) {
      std::cerr << "Error when creating SDL GL Context: '" << (SDL_GetError())
                << "'\n";
    }
...
struct program {
  ...
  program(int width, int height)
      : graphics_(width, height), game_(new game()) {}
};
...
int main(int argc, char *argv[]) {
  ...
  SDL_SetMainReady(); // Note: Emscripten crashes if SDL_INIT_TIMER is passed
                      // here
  ...
  // The mechanism of "emscripten_set_main_loop" is a bit opaque, better not to
  // make this a stack object.
